...
```bash
curl -X POST https://api.peeap.com/transactions/txn_abc123/capture \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "amount": 95.00
...
```bash
curl -X POST https://api.peeap.com/transactions/txn_abc123/void \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
```bash
curl -X POST https://api.peeap.com/transactions/txn_abc123/refund \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
